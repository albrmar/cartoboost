...
def test_huber_clips_outlier_influence_against_l2_python_backend() -> None:
...
def test_log_l2_rejects_invalid_targets_python_backend() -> None:
    X = np.arange(4, dtype=float).reshape(-1, 1)
    with pytest.raises(ValueError, match="log_l2 targets"):
        GeoBoostRegressor(loss="log_l2", backend="python").fit(X, [-1.0, 0.0, 1.0, 2.0])


def test_log_l2_predictions_return_original_scale_python_backend() -> None:
    X = np.arange(4, dtype=float).reshape(-1, 1)
