...
fn validate_multiclass_targets(targets: &[f64], class_count: usize) -> Result<()> {
    ...
    if targets.iter().any(|target| {
        target.fract() != 0.0 || *target < 0.0 || *target as usize >= class_count
    }) {
...
fn validate_prediction_shape(
    ...
    if raw_predictions.iter().any(|prediction| !prediction.is_finite()) {
        return Err(CartoBoostError::InvalidInput(
...
fn transform_predictions(
    ...
    match transform {
        ...
        PredictionTransformKind::Sigmoid => Ok(raw_predictions
            ...
            .collect()),
        PredictionTransformKind::Softmax => {
            if output_dimension < 2 || raw_predictions.len() % output_dimension != 0 {
                return Err(CartoBoostError::InvalidInput(
...
fn mean_ndcg(
    ...
    for range in &ranges {
        ...
        predicted
            .sort_by(|left, right| raw_predictions[*right].total_cmp(&raw_predictions[*left]));
        let dcg = predicted
            .into_iter()
            .take(limit)
            .enumerate()
...
mod tests {
    ...
    fn l2_objective_initializes_weighted_mean_and_residuals() {
        ...
        assert_eq!(
            objective.initial_margin(&targets, Some(&[3.0, 1.0])).unwrap(),
            vec![2.5]
        );
        assert_eq!(
            objective
    ...
    fn ranking_gain_treats_negative_relevance_as_zero() {
        ...
        assert_eq!(mean_ndcg(&targets, &bad_scores, Some(&[2]), Some(1)).unwrap(), 0.0);
    }
