...
def test_committed_parity_fixture_matches_saved_rust_artifact():
    ...
    model = GeoBoostRegressor.load(fixture_dir / fixture["model_path"])
...
def test_parity_fixture_generation_is_deterministic(tmp_path: Path):
    ...
    generated_model = GeoBoostRegressor.load(tmp_path / generated["model_path"])
    committed_model = GeoBoostRegressor.load(committed_dir / committed["model_path"])
