...
| `cartoboost_auto_forecast` | [AutoForecaster](user-guide/forecasting-models/auto-forecaster.md) | Rust-native `AutoForecastModel` facade over `cartoboost_lag`, `recency_weighted_lag`, `scaled_lag`, `delta_lag`, `scaled_delta_lag`, `seasonal_delta_lag`, `scaled_seasonal_delta_lag`, `ewm_lag`, dense-panel direct/rectified tree candidates, `log1p_scaled_lag`, `lag_plus`, sparse-panel `intermittent_demand`, and `classical_expert_bank`, using validation scores, lag baseline protection, hard-winner selection, and bounded close-race blends. | You want the default no-hyperopt guarded hybrid over a global lag spine, scaled lag, differenced lag, horizon-specific direct trees, log-scaled nonnegative lag, residual-corrected lag, sparse-demand specialists, and local statistical specialists. |
...
| `stl_cartoboost` | [Forecasting Decomposition](forecasting_decomposition.md) | Native additive STL decomposition with a native remainder forecaster and deterministic recomposition. | A strong taxi seasonality should be separated before modeling residual autocorrelation. |
