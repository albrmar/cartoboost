...
pub const WEIGHTS_ARTIFACT_TYPE: &str = "geoboost.weights";
...
pub fn load_weights_json(path: impl AsRef<Path>) -> Result<Model> {
    ...
    let artifact_type = value
        ...
        .ok_or_else(|| GeoBoostError::InvalidInput("missing weights artifact type".to_string()))?;
    ...
    let weights_artifact_version = value
        ...
        .ok_or_else(|| {
            GeoBoostError::InvalidInput("missing weights artifact version".to_string())
        })?;
