...
impl CategoricalEncoder {
    ...
    pub fn fit_transform_columns(
        ...
        let dataset = Dataset::from_rows(columns_to_rows(&encoded_columns)?)?
            .with_schema(FeatureSchema::numeric(encoder.encoded_feature_names.clone()))?;
        Ok((dataset, encoder))
    }
    ...
    pub fn transform_columns(&self, columns: &[Vec<String>]) -> Result<Dataset> {
        ...
        debug_assert!(encoded_columns.iter().all(|column| column.len() == row_count));
...
mod tests {
    ...
    fn low_cardinality_partition_encoding_adds_subset_indicators() {
        ...
        assert_eq!(encoder.columns[0].strategy, CategoricalEncodingStrategy::Partition);
